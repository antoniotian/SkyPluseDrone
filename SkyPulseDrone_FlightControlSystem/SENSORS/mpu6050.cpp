...
bool MPU6050::initialize()
{
    ...
    if(!i2cDevice->writeByte(PWR_MGMT_1, 0x00)){  // Wake up the device by writing 0 to the power management register.
        qWarning("MPU6050 wake up failed");
        return false;
    }
    if(!i2cDevice->writeByte(INT_ENABLE, 0x01)){  // Enable Data Ready interrupt
        qWarning("MPU6050 enable interrupt failed");
        return false;
    }
    if(!i2cDevice->writeByte(INT_PIN_CFG, 0x30)){  // Configure interrupt pin to be active high, push-pull, hold until cleared, clear on read
...
void MPU6050::collectCalibrationData()
{
